...
- All pre-granted application permissions are used via the configured cloud's Microsoft Graph host with the `/.default` scope.
...
Tokens are cached in a session-scoped temporary file (`os.TempDir()`) for the duration of the session. The cache is keyed by client ID, tenant ID, and cloud. No credentials are persisted permanently.
