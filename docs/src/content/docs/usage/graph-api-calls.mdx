...
# List users
msgraph-skill graph-call GET /users --select "displayName,mail" --top 10

# Get user's unread messages
msgraph-skill graph-call GET /me/messages --filter "isRead eq false" --top 5

# List groups
msgraph-skill graph-call GET /groups --select "displayName,description"
...
```bash
# Count users
msgraph-skill graph-call GET /users/\$count \
  --headers "ConsistencyLevel:eventual"

# Search users
msgraph-skill graph-call GET '/users?$search="displayName:John"' \
  --headers "ConsistencyLevel:eventual"
```
