---
title: How It Works
description: Technical details of how the agent skill operates
---

import MermaidDiagram from '../../../components/MermaidDiagram.astro';

This page explains the technical internals of msgraph for those who want to understand or extend it.

## Authentication Flow

### Delegated Auth

<MermaidDiagram chart={`
sequenceDiagram
    participant Agent as AI Agent
    participant CLI as msgraph CLI
    participant MSAL as MSAL Library
    participant Entra as Microsoft Entra ID
    participant Graph as Graph API

    Agent->>CLI: msgraph auth signin
    CLI->>MSAL: Acquire token (interactive)
    MSAL->>Entra: Authorization request
    Entra-->>MSAL: Auth code (via browser)
    MSAL->>Entra: Exchange code for tokens
    Entra-->>MSAL: Access + refresh tokens
    MSAL-->>CLI: Token cached
    CLI-->>Agent: Signed in ✓

    Agent->>CLI: msgraph graph-call GET /me
    CLI->>MSAL: Acquire token (silent)
    MSAL-->>CLI: Cached token
    CLI->>Graph: GET /me (Bearer token)
    Graph-->>CLI: 200 OK (user profile)
    CLI-->>Agent: JSON response
`} />

### App-Only Auth

<MermaidDiagram chart={`
sequenceDiagram
    participant Agent as AI Agent
    participant CLI as msgraph CLI
    participant MSAL as MSAL Library
    participant Entra as Microsoft Entra ID
    participant Graph as Graph API

    Note over CLI: Auto-detect auth method from env vars

    Agent->>CLI: msgraph auth signin
    alt Client Secret
        CLI->>MSAL: Acquire token by credential
        MSAL->>Entra: POST /token (client_credentials)
    else Certificate
        CLI->>MSAL: Acquire token by credential
        MSAL->>Entra: POST /token (client_assertion JWT)
    else Managed Identity
        CLI->>MSAL: Acquire token (resource)
        MSAL->>Entra: Token from IMDS endpoint
    else Workload Identity
        CLI->>MSAL: Acquire token by assertion
        MSAL->>Entra: POST /token (federated token)
    end
    Entra-->>MSAL: Access token
    MSAL-->>CLI: Token cached
    CLI-->>Agent: Signed in ✓

    Agent->>CLI: msgraph graph-call GET /users
    CLI->>MSAL: Acquire token (silent/credential)
    MSAL-->>CLI: Cached or fresh token
    CLI->>Graph: GET /users (Bearer token)
    Graph-->>CLI: 200 OK
    CLI-->>Agent: JSON response
`} />

### Auth Method Detection

The auth method is auto-detected from environment variables in priority order:

1. `MSGRAPH_CLIENT_SECRET` set → **Client secret**
2. `MSGRAPH_CLIENT_CERTIFICATE_PATH` set → **Certificate**
3. `MSGRAPH_FEDERATED_TOKEN_FILE` (or `AZURE_FEDERATED_TOKEN_FILE` / `AWS_WEB_IDENTITY_TOKEN_FILE`) set → **Workload identity**
4. `MSGRAPH_AUTH_METHOD=managed-identity` → **Managed identity**
5. None of the above → **Delegated** (interactive browser / device code)

### Silent Token Acquisition

After initial sign-in, tokens are cached. Subsequent calls use silent acquisition:

1. Check the in-memory/file cache for a valid token
2. If expired, MSAL automatically refreshes using the refresh token (delegated) or re-acquires via credentials (app-only)
3. Only if refresh fails does it prompt for interactive auth (delegated only)

### Incremental Consent (Delegated Only)

When a 403 Forbidden occurs with delegated auth:

1. Parse the error message for required scope names (regex: `[A-Z][a-zA-Z]+\.[A-Z][a-zA-Z]+`)
2. Merge new scopes with existing scopes
3. Re-authenticate with the combined scope set
4. Retry the original request with the new token

Incremental consent is not available for app-only auth — all permissions must be pre-configured and admin-consented in the Entra ID app registration.

## Safety Enforcement

Safety is enforced at the CLI level before any HTTP request is made:

| Method | Default | With --allow-writes |
|---|---|---|
| GET | Allowed | Allowed |
| HEAD | Allowed | Allowed |
| OPTIONS | Allowed | Allowed |
| POST | **Blocked** | Allowed |
| PUT | **Blocked** | Allowed |
| PATCH | **Blocked** | Allowed |
| DELETE | **Blocked** | **Blocked** |

DELETE is blocked unconditionally — there is no flag to enable it.

## Token Cache

The token cache uses MSAL's `ExportReplace` interface:

- **Export**: After any token acquisition, serialize the cache to a temp file
- **Replace**: Before any token lookup, deserialize from the temp file
- **Path**: `{os.TempDir()}/msgraph-session-{hash}.json`
- **Hash**: SHA-256 of `clientID:tenantID`, truncated to 8 bytes

The temp file allows multiple CLI invocations in the same session to share tokens without re-authenticating.

## OpenAPI Index

The index is built by `tools/openapi-indexer/`:

1. Downloads the [official Graph OpenAPI YAML](https://github.com/microsoftgraph/msgraph-metadata) (~60MB)
2. Parses using `gopkg.in/yaml.v3` with flexible `interface{}` unmarshaling
3. Extracts path, method, summary, description, and security scopes
4. Outputs a compact JSON file (~8MB, ~27,000 endpoints)

The search algorithm:
- Filters by method (exact match)
- Filters by resource (path segment match)
- Filters by query (all words must appear in path, summary, description, or scopes)
- Returns results with match reason

## Build System

- **Cross-platform**: Pre-built for macOS, Linux, and Windows (amd64 and arm64)
- **Cross-compilation**: 6 platform targets (darwin/linux/windows × amd64/arm64)
- **Docs site**: Astro + Starlight, deployed to GitHub Pages
- **Monorepo**: CLI source at root, npm workspace for docs
